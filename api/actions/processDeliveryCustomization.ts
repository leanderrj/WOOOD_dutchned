export const run: ActionRun = async ({ params, logger, api, connections }) => {
  const { orderId, cartItems } = params;

  if (!orderId) {
    throw new Error("orderId parameter is required");
  }

  // Get the current shop ID for tenant filtering
  ...
  for (const lineItem of lineItems) {
    ...
    try {
      // Fetch the product to get the shipping method metafield
      ...
      // Extract shipping method from metafield
      const shippingMethod = product.shippingMethod2;

      if (!shippingMethod) {
        logger.warn(`Product ${product.title} (${product.id}) missing shipping method metafield`);
        productsWithoutMethod++;
        continue;
      }

      // Parse the priority number from the metafield (e.g., "32 - EXPEDITIE 2-MANS")
      const priorityMatch = shippingMethod.match(/^(\d+)/);

      if (!priorityMatch) {
        logger.warn(`Invalid shipping method format for product ${product.title}: ${shippingMethod}`);
        productsWithoutMethod++;
        continue;
      }
      ...
      logger.info(`Product ${product.title}: priority ${priority}, method "${shippingMethod}"`);

      // Check if this is the highest priority so far
      if (priority > highestPriority) {
        highestPriority = priority;
        selectedShippingMethod = shippingMethod;

        // Create display name by removing the priority number and dash
        selectedMethodDisplay = shippingMethod.replace(/^\d+\s*-\s*/, '').trim();
      }
  ...
  if (selectedShippingMethod) {
    ...
    return {
      success: false,
      message: "No products with valid shipping methods found",
      stats: {
        productsProcessed,
        productsWithoutMethod
      }
