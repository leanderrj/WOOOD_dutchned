...
/**
 * Handle shipping method request and return formatted response
 * Main handler function for Workers endpoints
 */
export async function handleShippingMethodRequest(
  ...
  try {
    ...
    if (logger) {
      logger.error('Failed to handle shipping method request', {
        requestId,
        error: error.message,
        errorStack: error.stack,
        executionTime,
